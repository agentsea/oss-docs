...
In this guide, we'll show how to create your own agent using surfkit and some of the techniques we came up along the way to help your agent navigate GUIs and accomplish their goals. 
...
that allows it to control the VM (in the cloud or a local one) via a mouse and a keyboard programmatically. You just created it above with `surfkit create device`.
...
ask an MLLM/LLM to return the next action that format, and then on each step of solving a task, 
...
When this object gets returned to device, the device can execute it: in this case, type the text "cool cat images" using the keyboard.

The actions include the low-level operations with a mouse and a keyboard, like moving mouse, clicking on coordinates, typing letters. They also include taking the screenshot and getting the current mouse coordinates, which helps an MLLM to choose the next action towards the task completion.
...
So it's time to make this agent better with some additional tricks.  Our gaol is to help them  **convert an idea on what should be 
...
As you can see, it is not so smart at the moment: it simply returns the coordinates at middle of the screen. Don't worry, we'll work on it later!
...
None of them are perfect (to the best of our knowledge at the moment of writing this tutorial), but combining a few in one agent can guarantee pretty high accuracy. Let's start with something simple.
...
When you run the agent now, you can see the images with the grid that it generates, in the debug tab. The MLLM picks the correct number pretty reliably. This method is obviously more intelligent than picking the middle of the screen. However, there is a good change to miss the correct spot because the element we're interested it is right under the middle of the dot.
...
When we have this, we update `click_object`. We move the grid-related logic to a separate method, add a similar one with the OCR-related logic, and update the main action method as this:
...
We strongly believe that the key of the success of the agent is mixing and matching a bunch of techniques, including everything from classical ML to deep learning to the frontier models, spiced up with the traditional programming stuff. So get in there and try your own techniques! Get creative. 
