---
title: (Review) Quickstart
description: Start using agents in under 5 minutes
icon: rocket
---

## Prerequisites

- Docker
- Python >= 3.10
- MacOS or Linux

## Install Surfkit

```
pip install surfkit
```

## Create an Agent

**Find available agents on the Hub**

```
surfkit find
```

**Create a new agent**

Replace "/my/path/" with your local path

```
surfkit create agent -t /my/path/SurfPizza -n agent01
```

**List running agents**

```
surfkit list agents
```

<AccordionGroup>

<Accordion icon="sailboat" title="Create an agent on Kubernetes">
To run the agent on Kubernetes all you need is a working kubeconfig

```
surfkit create agent -t /my/path/SurfPizza --runtime kube
```

</Accordion>
<Accordion icon="key" title="Automatically add local API keys">
Agents will prompt you for any keys they need. In order to skip this and automatically add local keys
```
surfkit create agent -t /my/path/SurfPizza --local-keys
```

</Accordion>
<Accordion icon="file" title="View agent logs">
View all agent logs
```
surfkit logs agent <name>
```

Follow logs

```
surfkit logs agent <name> --follow
```

</Accordion>
</AccordionGroup>

## Create a Device

Create an Ubuntu desktop for our agent to use.

Select one of the following options ▼

<AccordionGroup>
<Accordion icon="q" title="Create a local desktop using Qemu">
__Requirements__
- Qemu installed ([macOS](https://www.qemu.org/download/#macos), [linux](https://www.qemu.org/download/#linux))
- At least 16GB of RAM

```
surfkit create desktop --provider qemu -n desktop01
```

</Accordion>

<Accordion icon="google" title="Create a desktop on GCE">
__Requirements__
- [gcloud](https://cloud.google.com/sdk/docs/install) installed
- [Logged in](https://cloud.google.com/sdk/gcloud/reference/auth/application-default/login) to an account with create permissions for GCE

```
surfkit create desktop --provider gce -n desktop01
```

</Accordion>
<Accordion icon="aws" title="Create a desktop on EC2">
__Requirements__
- [Authentication configured](https://docs.aws.amazon.com/cli/v1/userguide/cli-chap-authentication.html) with create permissions for EC2

```
surfkit create desktop --provider ec2 -n desktop01
```

</Accordion>

</AccordionGroup>

List running devices

```
surfkit list devices
```

## Solve a task

Use the agent to solve a task on the device

```
surfkit solve "Search for the most common variety of french duck" \
  --agent agent01 \
  --device desktop01
```

## What's next

Now that you have a feel for the basics, you can learn more about the tools surfkit provides.

<CardGroup>

<Card title="Create an Agent" icon="paintbrush" href="/advanced_tutorial">
  Create your own agent
</Card>

<Card title="Learn more" icon="wrench" href="/introduction">
  Learn more about the tools
</Card>

</CardGroup>
