...
often preferred to run the VM in the cloud. This section will walk you through how to set up a GCP account to run a VM from scratch.
...
NOTE: If your already have other gcloud accounts configured for your work or for other projects you should see something like the following:
...
``` 
NOTE: The easiest is to choose [2] and create a new configuration to ensure you keep your old configurations. However, if you do not wish to keep old accounts you can choose to [1] to re-initialize the settings.

If you do that you will also see the following prompt:
...
Choose 2 to use your new account.
...
Now create a VM for an agent to use

```
surfkit create device --provider gce
```

## Configure AWS to create desktop VMs

Agents are designed to be capable of operating on desktop VMs. Running a full VM locally can consume a fair amount of resources, it's
often preferred to run the VM in the cloud. This section will walk you through how to set up a AWS account to run a VM from scratch.
...
```
surfkit create device --provider ec2
```
